    ...
    def _log_zero_history(self, previous_positions):
        ...
        try:
            ...
            record = {
                ...
                "previous_angle_deg": [
                    round(float(a), 4)
                    for a in self.position_to_angle(previous_positions)
                ],
                "revert_position": [
                    2 * ENCODER_CENTER - p for p in previous_positions
                ],
            }
            history_file = Path(STATE_FOLDER) / "set_zero_history_{:s}.jsonl".format(
                str(self.board_id)
            )
